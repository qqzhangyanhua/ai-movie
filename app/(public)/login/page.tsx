        ...
        <Card className="w-full bg-[#0A0A0A] border-white/10 shadow-2xl p-6 sm:p-8 rounded-xl backdrop-blur-sm">
          <CardHeader className="space-y-4 pb-6 p-0 text-center flex flex-col items-center">
            ...
            <div className="space-y-1">
              ...
              <CardDescription className="text-sm text-gray-400">
                Log in to continue to AI Movie
              </CardDescription>
            </div>
          </CardHeader>
          ...
          <form onSubmit={handleSubmit}>
            <CardContent className="space-y-4 p-0">
              {registered && (
                <div className="rounded-md border border-[#D4AF37]/30 bg-[#D4AF37]/10 px-3 py-2 text-xs text-[#D4AF37] text-center mb-4">
                  注册成功，请登录
                </div>
              )}
              {error && (
            ...
            <CardFooter className="flex flex-col gap-4 p-0 pt-6">
              <Link href="/register" className="text-center text-xs text-gray-500 hover:text-white transition-colors">
                Don&apos;t have an account? <span className="text-white hover:underline">Sign up</span>
              </Link>
            </CardFooter>
          </form>
        </Card>
  ...
  return (
    <Suspense fallback={
      <div className="flex min-h-screen items-center justify-center p-4 bg-[#050505]">
        <div className="w-full max-w-sm h-[500px] animate-pulse rounded-xl bg-[#0A0A0A] border border-white/10" />
      </div>
    }>
      <LoginForm />
    </Suspense>
